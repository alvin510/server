    ...
    try {
      ...
      if (req.files["profile"] && req.files["profile"][0]) {
        const newProfile = new Profile({
          ...
          keterangan,
          sejarah,
          profile: req.files["profile"][0].filename, // Menambah sejarah ke data yang disimpan
        });
        const saveProfile = await newProfile.save();
        res
          .status(200)
          .json({ profile: saveProfile, message: "Profile telah disimpan" });
      } else {
...
route.get("/get-profile", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Terjadi kesalahan saat mengambil data profile." });
  }
});
...
route.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Terjadi kesalahan saat memperbarui profile." });
    }
  }
);
...
route.delete("/delete-profile/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Terjadi kesalahan saat menghapus data profile." });
  }
});
